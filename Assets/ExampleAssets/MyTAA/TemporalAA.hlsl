...
half2 GetVelocityWithOffset(float2 uv, half2 depthOffsetUv)
{
    ...
    half2 offsetUv = SAMPLE_TEXTURE2D_X(_MotionVectorTexture, sampler_LinearClamp,
                                        uv + _MotionVectorTexture_TexelSize.xy * depthOffsetUv).xy;
    return -offsetUv;
}
...
void AdjustBestDepthOffset(inout half bestDepth, inout half bestX, inout half bestY, float2 uv, half currX, half currY)
{
    ...
    half depth = SAMPLE_TEXTURE2D_X(_CameraDepthTexture, sampler_PointClamp,
                                    uv.xy + _BlitTexture_TexelSize.xy * half2(currX, currY)).r;
...
void AdjustColorBox(inout half3 boxMin, inout half3 boxMax, inout half3 moment1, inout half3 moment2, float2 uv,
...
half4 DoTemporalAA(Varyings input, int clampQuality, int motionQuality, int historyQuality, int centralFiltering)
{
    ...
    half3 accum = (historyQuality >= 2)
                      ...
                      : SceneToWorkingSpace(
                          SAMPLE_TEXTURE2D_X(_TaaAccumulationTex, sampler_LinearClamp, historyUv).xyz);
